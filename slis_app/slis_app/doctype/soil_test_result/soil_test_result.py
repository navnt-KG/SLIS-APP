    ...
    def autoname(self):
        ...
        if not lab_code:
            frappe.throw(
                ...
                "for your Employee record to generate an ID."
            )
...
def generate_recommendation(docname):
    ...
    # -----------------------------------
    # STEP 2 - Find Fertility Adjustment
    # -----------------------------------
    adjustments = frappe.get_all("Fertility Adjustment Table", fields=["*"])
    ...
    # -----------------------------------
    # STEP 3 - Get Crop List
    # CHANGED: use main_sample_id → Soil Sample Collection
    # -----------------------------------
    if not doc.main_sample_id:
        frappe.throw("No Soil Sample Collection linked to this Soil Test Result")

    collection = frappe.get_doc("Soil Sample Collection", doc.main_sample_id)

    if not collection.crops_list:
        frappe.throw(f"No crops found in Soil Sample Collection {doc.main_sample_id}")

    doc.recommendations_table = []

    # -----------------------------------
    # STEP 4 - Process Each Crop
    # -----------------------------------
    for crop_row in collection.crops_list:
        ...
        # Read PoP Nutrient Values
        for nutrient in pop.nutrient_data:
            if nutrient.nutrient and "Available Nitrogen" in nutrient.nutrient \
                    and organic_carbon is not None:
                n_required = float(nutrient.quantity) * n_adjust / 100
                n_unit     = nutrient.unit or ''

            elif nutrient.nutrient and "Available Phosphorous" in nutrient.nutrient \
                    and available_p is not None:
                p_required = float(nutrient.quantity) * pk_adjust / 100
                p_unit     = nutrient.unit or ''

            elif nutrient.nutrient and "Available Potassium" in nutrient.nutrient \
                    and available_k is not None:
                k_required = float(nutrient.quantity) * pk_adjust / 100
                k_unit     = nutrient.unit or ''

        # -----------------------------------
        # STEP 5 - Get Fertilizers
        # -----------------------------------
        fert_n_name = frappe.db.get_value(
            "Fertilizer",
            {"primary_nutrient": ["like", "%Available Nitrogen%"]},
        ...
        fert_n = frappe.get_doc("Fertilizer", fert_n_name) if fert_n_name else None
        fert_p = frappe.get_doc("Fertilizer", fert_p_name) if fert_p_name else None
        fert_k = frappe.get_doc("Fertilizer", fert_k_name) if fert_k_name else None

        # -----------------------------------
        # STEP 6 - Convert Nutrient → Fertilizer
        # -----------------------------------
        urea    = 0
        rajphos = 0
        potash  = 0
